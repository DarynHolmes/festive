...
2. Cache invalidation — tested through Sprint 1 (optimistic updates with targeted `setQueryData`) and Sprint 3 (offline queue replay with cache patching). No issues.
3. No blocking bugs encountered

**Status: Pinia Colada retained.** Mutations, optimistic updates, and offline queue replay all work well with its API. No reason to switch.
