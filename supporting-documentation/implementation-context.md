...
**Last updated:** Sprint 5 (Test Dashboard & Quality Visibility) complete.
...
## CI & Quality Visibility

- **CI pipeline:** `.github/workflows/ci.yml` — lint, unit tests, build, Playwright E2E on every push to `main` and all PRs
- **Test dashboard:** `.github/scripts/ci-report.mjs` generates a self-contained HTML dashboard from vitest JSON, Playwright JSON, and ESLint JSON reports; deployed to GitHub Pages at `https://darynholmes.github.io/festive/`
- **Playwright report:** Deployed alongside dashboard at `./playwright-report/`
- **Deploy:** `peaceiris/actions-gh-pages@v4` pushes `gh-pages-deploy/` directory to GitHub Pages on every push to main
